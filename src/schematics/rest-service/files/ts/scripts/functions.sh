...
# DESC: Parameter parser ( all scripts must use the same argument structure)
...
# DESC: Import/source a bash file and execute the main function
# ARGS: $1 (required): path of script
# OUTS: None
function import() {
    #local __source="$(dirname "${BASH_SOURCE[0]}")/$1"
    #local __source="${BASH_SOURCE[0]}/$1"
    local __source="$1"
...
function begin() {
    pretty_print "[begin] $script_name" $fg_blue
}

function end() {
    pretty_print "[end] $script_name" $fg_blue
}
...
function import_env_vars() {
    ...
    if [[ "${CLUSTER_ENV:-}" == "AWS" ]]; then
        ...
        if [ -f "./config/credentials" ]; then
            ...
            pretty_print "ERROR: the credentials file is missing from the config folder. Please add this file and try again." $fg_red
            exit 1
        fi
    fi
}
